...
class OllamaMenuClient:
    ...
    async def section_per_context(self, data: str, menu:dict):
        ...
        2번 프로세스) unsplash, pixabey와 같은 이미지 검색 사이트에서 이미지가 잘 검색될 영문 키워드 2개를 선정하여 작성하세요.
        ...
        1. **섹션 구조**는 다음과 같이 메뉴 이름(키) 목록을 가집니다:
        {menu}
        2. 입력 데이터({{data}})를 고려하여 범용적인 키워드를 1개 지정해줘.
         - 각 키워드는 최대 3개의 단어로 이루어져있어.
         - 여기서 선정된 키워드는 모든 메뉴에 공통적으로 들어갈거야.
         - 오탈자가 없게 작성해줘.
        3. 1번 프로세스)의 결과물({{menu_structure}})을 참고해서 웹사이트에 들어갈 이미지가 나올만한 키워드를 1개 선정해줘.
         - 각 키워드는 최대 3개의 단어로 이루어져있어.
         - 여기서 선정된 키워드는 각각의 메뉴에만 들어갈거야.
         - 오탈자가 없게 작성해줘.
        4. **JSON 형식 이외의** 어떤 설명, 문장, 주석, 코드 블록도 작성하지 마세요.
        5. 최종 출력은 반드시 **오직 JSON 구조**만 반환해야 합니다.
        ...
        - 예시:
                ...
                keyword_structure : {{
                    "keyword_Hero": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_Feature": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_Content": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_Testimonial": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_CTA": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_Pricing": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_Contact": "2번 프로세스) 규칙을 준수하여 작성",
                    "keyword_Footers": "2번 프로세스) 규칙을 준수하여 작성"
                }}
