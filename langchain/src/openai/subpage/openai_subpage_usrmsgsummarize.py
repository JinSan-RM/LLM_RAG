...
class OpenAIUsrMsgProposalGenerator:
    ...
    async def usr_msg_proposal(self, subpage_n_prompt, main_context, max_tokens: int = 500) -> dict:
        try:
            subpage = ""
            prompt = ""
            for subpage, prompt in subpage_n_prompt.items():
                subpage = subpage
                prompt = prompt
            
            # NOTE 250501 : 좀 더 목적에 맞게 프롬프트 수정
            sys_prompt = f"""
                You are a professional in website content planning and development. 
                You are provided with user input containing details about a corporate website project, including specific subpages to create, user requirements, and main page text. 
                Your task is to create a structured guide for organizing this information to facilitate the creation of subpages for a corporate introduction website.

                #### INSTRUCTIONS ####

                1. Do not expand the scope beyond the provided input unless necessary to fill gaps logically.
                2. Identify and extract key information from the user input:
                    - Subpage types: {{subpage}} (e.g., About Us, Services, Team, Contact).
                    - User requirements: {{prompt}} (e.g., content to emphasize, such as innovation or client success).
                    - Main page text: {{main_context}} (e.g., mission statement, tone, or reusable content).
                3. Process the information step-by-step to create a clear organizational plan:
                    - Categorize the subpage types ({{subpage}}) by purpose (e.g., informational, promotional) and target audience (e.g., clients, partners).
                    - Map the user requirements ({{prompt}}) to relevant subpages and suggest methods to highlight them (e.g., hero sections, testimonials, visuals).
                    - Analyze the main page text ({{main_context}}) for reusable content, tone, and gaps to address in subpages.
                4. Produce a concise output (800-1000 characters) that includes:
                    - A brief overview of the processing approach.
                    - Specific steps for handling subpage types ({{subpage}}), user requirements ({{prompt}}), and main page text ({main_context}).
                    - Recommendations for aligning subpages with the website’s goals and ensuring consistency with the main page.
                5. Use clear, varied, and professional language to ensure the output is actionable and avoids redundancy.
                6. If any input ({{subpage}}, {{prompt}}, {{main_context}}) is incomplete, make reasonable assumptions based on typical corporate website needs.

                #### Output Language ####
                **{self.output_language}**         
            """

            usr_prompt = f"""user_input: 
                            {{Subpage types: {subpage}. 
                            User requirements: {prompt}. 
                            Main page text: {main_context}}}"""
